...
def defaults():
    ...
    env.service_name = ["gunicorn-django_falcor_test"]
    env.code_dir = '/srv/django_falcor_test'
...
def deploy(id=None, silent=False, force=False):
    ...
    package_changed = force or vcs.changed_files(revset, r' django_falcor_test/package.json')
    if package_changed:
        print colors.yellow("Will run npm install")

    # See if we have changes in app source or static files
    app_changed = force or vcs.changed_files(revset, [r' django_falcor_test/app', r' django_falcor_test/static', r' django_falcor_test/settings', r'webpack'])
    if app_changed:
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/django_falcor_test')
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE django_falcor_test; '
         '      CREATE USER django_falcor_test WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE django_falcor_test to django_falcor_test;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/django_falcor_test/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/django_falcor_test/')
    with cd(env.code_dir + '/django_falcor_test'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/django_falcor_test/')

    # Copy logrotate, nginx, crontab and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/django_falcor_test')
        sudo('cp deploy/%s /etc/nginx/sites-enabled/django_falcor_test' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-django_falcor_test.conf')
        sudo('cp deploy/crontab.conf /etc/cron.d/django_falcor_test')
...
def collectstatic(npm_install=True, npm_build=True):
    with cd(env.code_dir + '/django_falcor_test'):
        if npm_install:
